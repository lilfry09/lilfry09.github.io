...
IDBD 还为每个参数 $i$ 维护一个 log step-size $\beta_{i,t}$，令
...
关键在于 $\beta_{i,t}$ 也会根据“改变该参数的步长是否让未来损失下降”而更新。为此算法维护一个与参数同形状的 eligibility / sensitivity 状态 $h_{i,t}$，用它近似当前参数对历史预测的影响。不同论文对 $h_{i,t}$ 的具体递推和符号约定略有差异，但抽象结构可以写成：
...
其中 $\rho_t$ 控制历史影响的衰减，$\eta$ 是 meta step-size。直觉是：若某个输入通道的更新长期能降低可预测误差，$\beta_i$ 会保持较大；若它只响应随机噪声，继续增大步长不会带来稳定收益，$\beta_i$ 会下降。
...
这条路线的成败取决于中间环节能否真正闭环。若 NetworkIDBD 只能在受控任务上过滤噪声，无法在复杂视觉或交互环境中稳定追踪；若事件驱动网络节省了算力，却牺牲了表示质量；若 OaK 的抽象不能被规划器可靠调用，那么“从经验到持续学习与规划”的链条就会在某一层断开。
...
> Oak Lab 想解决的不是“如何把 curated dataset 做得更大”，而是“当世界本身就是噪声、变化和不可预测时，智能体如何只学习值得学习的东西，并在给定计算与能耗预算下持续行动”。
