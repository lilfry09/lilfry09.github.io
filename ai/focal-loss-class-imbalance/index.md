...
如果你发现模型在多数类上表现很好，但在少数类上召回率很差，或者模型训练过程中负样本损失占据主导，那么 Focal Loss 可以作为一个对照实验；是否保留它，要看少数类 recall、F1 和 PR-AUC 是否在相同训练预算下改善。
...
它通过引入 $(1 - p_t)^\gamma$ 这个调节因子，动态降低容易样本的损失贡献；再通过 $\alpha$ 平衡不同类别的重要性。对于目标检测、异常检测、医学影像、缺陷识别等任务，它可以作为针对类别不平衡的 baseline 变体，但仍需要和 Cross Entropy、重采样或类别权重在同一验证集上比较。
